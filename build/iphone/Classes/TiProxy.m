...
//Rare exceptions to indicate a bug in the _rssreader code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_rssreader = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_rssreader]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.rssreader.module"
